...
export default function ActorSinglePage({ movie }) {
    console.log(movie)
    const removeHashtags = (text)=> text.replace(/#[^\s]+/g, '') // remove hashtags from text
    const listGenres = (genres)=> genres.map((genre)=>{
        return <span key={genre.id}> <Link href={"/genres/"+genre.name}>{genre.name}, </Link> </span>
    })
